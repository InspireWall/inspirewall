...
## Moving existing images into `images/` folder (optional)

If you prefer to centralize wallpaper images under a top-level `images/` folder, this project includes a helper script that will copy the current `assets/Images/*` files to `images/` and update the JSON manifest.

```powershell
# copy images to images/ and regenerate the manifest
node scripts/copy-images-to-images.js
node scripts/generate-wallpapers-json.js
```

After this runs, the manifest `wallpapers.json` will reference `images/` paths and the showcase will pick them up automatically.
